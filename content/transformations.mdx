...
![Editing an endpoint's Transformation](/img/transformations-edit.png)

![Testing an endpoint's Transformation](/img/transformations-test.png)
...
- `method`, a string representing the HTTP method the webhook will be sent with. It is always `"POST"` by default, and its only valid values are `"POST"`, `"PUT"`, or `"PATCH"`.
...
- `env`, (string) key to (string) value map of variables defined in the 'Variables' tab. Changes are ignored.
