...
- `transformationsParams`, additional parameters made available in ingest; se below for details
...
- `transformationsParams.headers` will be a string/string key-value map containing the headers of the input request. If a header is duplicated on the input request, one will be chosen arbitrarily for inclusion in the map.
