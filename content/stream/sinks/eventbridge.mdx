...
You can authenticate sinks to EventBridge using either a fixed AWS Access Key ID and Secret Access Key, or by using a role and cross-account delegated authentication. In either case, the target user/role must have the `events:PutEvents` permission on the relevant Event Bus.
