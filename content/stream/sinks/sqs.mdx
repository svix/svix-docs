...
You can authenticate sinks to SQS using either a fixed AWS Access Key ID and Secret Access Key, or by using a role and cross-account delegated authentication. In either case, the target user/role must have the `sqs:SendMessage` permission on the relevant queue.
