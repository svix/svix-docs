...
Once you've created some applications you can list them with via the `list` command:
...
Next we can add an endpoint to the application we created above.  The create command should have returned an Application ID.
Since the application name is not unique, will need its ID to make changes.

Lets add a webhook endpoint with the url `http://example.com/webhook`. Every endpoint must have a version associated with it
...
Now that we have an application created and and enpoint configured to listen to messages from that app, we are ready to send our first
webhook message! Lets send a `user.created` event with the users name and email.
