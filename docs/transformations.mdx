...
If you are using the Slack built-in integration, structure your JSON payload according to [Slack's Incoming Webhook API](https://api.slack.com/messaging/webhooks). For Discord, use [Discord's Webhook API](https://discord.com/developers/docs/resources/webhook#execute-webhook).
...
This is a way for your users to receive events as Slack messages in a few clicks, without thinking about webhooks at all.

To use the Slack integration portal, create a Slack transformation template, use the transformation code to define
how you want to format the Slack message for each of your event types.

To use the portal, get the App Portal URL and add the `mode=slack-button` query parameter (like `app.svix.com/login?mode=slack-button#key=<login-key>`).

Add a button to your site, you can call it something like 'Configure Slack'. When the button is clicked, redirect the user to the Slack portal URL.
...
In the portal, your users can enable and disable the connection, and choose which events they want to receive. If you're using a popup, it will automatically close
