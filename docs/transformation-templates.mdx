...
- `type` - Choose from a set of services that provide easy webhook integrations, or choose `Custom` to build your own integration. We currently support Slack, Discord, and many more services.
- `Instructions` and `Learn more link` - Only visible for `Custom` templates. Explain how to use and customize the template. Your customers may need to set a specific endpoint when using the template. This is a good place to let them know how to get that endpoint.
...
The Slack built-in integration lets you send messages to Slack channels via webhooks. It renders a 'Connect to Slack' button that lets your users get a Slack incoming webhook URL using OAuth in a few clicks, without leaving your site.
...
The Discord built-in integration lets you send messages to Discord channels via webhooks. It renders a 'Connect to Discord' button that lets your users get a Discord incoming webhook URL using OAuth in a few clicks, without leaving your site.
...
The Microsoft Teams integration will show instructions on how to get a Teams webhook URL. 
...
The Hubspot integration lets you connect webhooks directly to the [Hubspot API](https://developers.hubspot.com/docs/api/overview). It renders a 'Connect to Hubspot' button that lets your users get a Hubspot access token that can be used to connect to the Hubspot API.
...
// TODO: screenshot with fake service
