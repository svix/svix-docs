...
Each of your users needs an associated application. The easiest way is to create a new application whenever a user signs up. In this section we will use the [create application API endpoint](https://api.svix.com/docs#/application/create_application_api_v1_app__post) to create an application.
...
We will now send a new message using the [create message API endpoint](https://api.svix.com/docs#/message/create_message_api_v1_app__app_id__msg__post). It accepts an `app_id`, which is the application's ID from the previous section. In addition, it accepts the following fields (as part the JSON body):
...
Svix offers a pre-build management UI. With [one API call](https://api.svix.com/docs#/authentication/get_dashboard_access_api_v1_auth_dashboard_access__app_id___post), you can give your users access to this UI and they can then add their own endpoints themselves.

More on that in the [management UI docs](management-ui).

### Using the API

In addition to the management UI, you can also use our API to add endpoints to your applications. To do so, you will use the [create endpoint API](https://api.svix.com/docs#/endpoint/create_endpoint_api_v1_app__app_id__endpoint__post).
