...
Each of your users needs an associated consumer application. The easiest way is to create a new application whenever a user signs up, though you can also create it when your customers enable webhooks, or have Svix create these automatically (more on that later).
...
Here is how to create an application using the [create application API endpoint](https://api.svix.com/docs#operation/create_application_api_v1_app__post):
