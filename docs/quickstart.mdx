...
val applicationOut = svix.application.create(ApplicationIn("Application name"))
```
...
svix.message.create("app_Xzx8bQeOB1D1XEYmAJaRGoj0",
        MessageIn("invoice.paid",
                mapOf<String, Any>(
                        "id" to "invoice_WF7WtCLFFtd8ubcTgboSFNql",
                        "status" to "paid",
                        "attempt" to 2
                ),
                "evt_Wqb1k73rXprtTm7Qdlr38G"))
```
...
val svix = Svix("AUTH_TOKEN")
val endpointOut = 
    svix.endpoint.create("app_Xzx8bQeOB1D1XEYmAJaRGoj0", EndpointIn("https://api.example.com/svix-webhooks/", 1, "My main endpoint"))
```
