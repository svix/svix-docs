---
title: How to Verify Webhooks with ngrok
---
...
When you run the command, ngrok should generate URL that looks like `https://*.ngrok.io`. Copy that URL, and set it as the URL for the Svix endpoint you previously created in the Consumer App Portal.

Assuming you are running a service on port 3000, all Svix webhooks to your endpoint will now be forwarded to that local service by ngrok. And because you configured the `--verify-webhook` and `--verify-webhook-secret` options, ngrok will only accept verified Svix webhooks.
