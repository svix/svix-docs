...
You also have the option of creating an application the first time you send a message to it. When sending a message you can, optionally, send an application specification along with it. If the application does not exist yet it will be created; if it does then the application specification is ignored.
