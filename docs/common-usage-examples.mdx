...
This use-case is the right one for you if you have different customers on your service, and each of get sent webhooks independently based on activities on their own separate accounts. This is the most common use-case and probably matches most people.

One company that matches this description is Stripe. Every Stripe customer has their own account, and webhooks are sent by Stripe to a specific account. The webhooks may cover activity of many of this custom's users (e.g. whether they paid), but still only affecting this one customer. Another such example is [Clerk](https://clerk.dev), where each of their customers may subscribe for webhooks about the activity of their own account.


#### Usage example

A service matching this description should create one application for each one of their customers. The service will then send events related to this specific customer to the application, and offer the [application portal](./app-portal.mdx) to each of their customers so that they can manage their webhooks.

As mentioned above, the service can utilize event types to let their customers filter which event they would like to listen to, and it can also utilize application `UID`s to [use Svix in a fully stateless manner](./overview.mdx#ids-and-uids).
...
One easy solution is to have one different application per each project. While it's possible to do it this way, it comes with a few annoying limitations. First of all, the app portal only shows one application at a time, so managing the endpoints is annoying. But also, oftentimes you'd like to listen for events affecting multiple projects from the same endpoint, and splitting it to different applications makes this very cumbersome.

Enter [channels](./channels.mdx). Channels are an extra dimension of filtering messages that is similar, but orthogonal to event types. Channels let you include in the message which channels have been affected by a particular change. For example, you can send a msg and mark it as affecting `proj_12` and `proj_35`. Endpoints can then be marked as listening to only changes affecting particular projects (channels) and the messages will be filtered accordingly.
...
This use-case is for services where the webhooks are not being ingested by the service's customers, but rather by partners that e.g. build extensions for it.
...
The first method is to have one application per each of your customers (even though they are unaware of webhooks), and for every partner that would like to listen to webhooks add a new endpoint with this partners' URL and event type requirements (with a unique endpoint `UID` to [keep things stateless](./overview.mdx#ids-and-uids)). The nice thing about this method is that it keeps things simple and all of the webhooks for one application are sent to one place. The bad thing, however, is that if this partner ever wants to change the URL or the event type, this needs to be updated for each and every integration that they use.

The second method is to have one application per one partner, and then when an event happens on a customer that a partner is interested in send a message to this partner's app (and the same for any other partner listening). This is currently done with separate API calls, but we plan on adding a way to mass send a message to multiple applications, so please [let us know](./get-help.mdx) if this is something you would be interested in.
...
One easy solution is to just have the equivalent of multiple use-cases layered on top of one another. So literally just send messages to multiple different applications that are exposed different in the your service's dashboard to different partners and customers based on their roles.
