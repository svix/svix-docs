...
Another easy way to create or update event types is by using the OpenAPI specification for your API, if you have one. 

You can upload your spec JSON file in the dashboard, and we'll read the [`webhooks`](https://github.com/OAI/OpenAPI-Specification/blob/main/versions/3.1.0.md#fixed-fields) field (`x-webhooks` in OpenAPI 3.0) 
and suggest event types ([and their JSON schemas](/event-types#event-type-schema)) based on the `body` of your webhook requests.

![event-type-openapi](./img/event-type-openapi.png)

You can also add a `x-svix-eventTypeId` property as part of your webhook [Path Item](https://github.com/OAI/OpenAPI-Specification/blob/main/versions/3.1.0.md#path-item-object), and we'll automatically use it to set the Svix event type name. 
...
Customers on the Professional and Enterprise tiers can also have the Event Catalog served from their own domain, such as **webhooks.example.com** rather than the default **www.svix.com**. Please [contact us](https://www.svix.com/contact/) and we'll help you set it up.
