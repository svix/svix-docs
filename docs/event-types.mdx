...
Another easy way to bulk create or update your event types is by using the OpenAPI specification for your API, if you have one. 
...
and suggest event types ([and their JSON schemas](http://localhost:3000/tutorials/event-type-schema#option-b-providing-a-jsonschema)) based on the `body` of your webhook requests.

![event-type-openapi](./img/event-type-openapi.png)

You can also add `x-svix-eventTypeId` custom propery as part of your webhook Path Item, and we'll automatically use it to set the Svix event type name. 
