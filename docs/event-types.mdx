...
svix.eventType.create(
                EventTypeIn("A user has signed up", "user.signup"));
```
...
internal class App {
    suspend fun main() {
        val svix = Svix("AUTH_TOKEN")
        try {
            Scanner(FileReader("/Users/fcjr/Desktop/import/data.csv")).use { scanner ->
                while (scanner.hasNextLine()) {
                    try {
                        val line: List<String> = scanner.nextLine().split("|")
                        svix.eventType.create(EventTypeIn(line[1], line[0]))
