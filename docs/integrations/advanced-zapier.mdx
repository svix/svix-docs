...
#### Linking the integration to your new local package

Follow the same instructions as in [Build a Zapier Integration](./zapier) guide except instead of calling the `zapier register` command to create a new integration, use the [link command](https://platform.zapier.com/cli_docs/cli#link) to link an existing one.
```
zapier link
```
...
If you wish to version bump your integration, be sure to update your integration's `package.json` to reflect the new version number. It defaults to `1.0.0`. Review the [Zapier Platform](https://platform.zapier.com/cli_docs/docs#deploying-an-app-version) docs for more infromation on versioning.
