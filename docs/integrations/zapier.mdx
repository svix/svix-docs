...
The [Svix Integrations API](https://api.svix.com/docs#tag/Integrations) is used to manage Integrations and generate long-lived rotatable credentials to other services. This enables other services to create and manage endpoints. Each service your users connect with, including Zapier, should have a corresponding Integration. 

By default, the auto-generated Zapier integration requires the Application ID and Integration Key. You will need to generate and provide both to your users. To avoid exposing these constructs to your users, you can configure OAuth2 or Session Auth with Zapier. More information on this is provided below.
...
With either scheme, you can send the Application ID and Integration Key back with the trigger for use by the triggers. That way, your users will interact with Svix fully transparently.
