...
Sinks define where Events in the Stream are sent, and can be anything from an [HTTP endpoint](./sink-types/http) to an Object Storage bucket ([S3](./sink-types/s3), [Google Cloud Storage](./sink-types/gcs), [Azure Blob Storage](./sink-types/azure_blob)), or even an [Open Telemetry Trace collector](./sink-types/otel_trace).
