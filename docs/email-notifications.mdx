...
ApplicationOut app = svix.getApplication().create(
    new ApplicationIn().name("Application name").uid("example-customer-123").metadata(metadata)
);
```
...
const svix = new Svix("AUTH_TOKEN");
await svix.application.patch("user-123", {
  metadata: {
    "svix.email": "user-123@example.com"
  }
});
```
...
svix = Svix("AUTH_TOKEN")
svix.application.patch("user-123", {
    "metadata": {
        "svix.email": "user-123@example.com"
    }
})
...
app, err := svixClient.Application.Patch(ctx, "user-123", &svix.ApplicationPatch{
    Metadata: &metadata,
})
```
...
let svix = Svix::new("AUTH_TOKEN".to_string(), None);
let mut metadata = std::collections::HashMap::new();
metadata.insert("svix.email".to_string(), serde_json::Value::String("user-123@example.com".to_string()));
...
Svix svix = new Svix("AUTH_TOKEN");
Map<String, Object> metadata = new HashMap<>();
metadata.put("svix.email", "user-123@example.com");

svix.getApplication()
  .patch("user-123", new ApplicationPatch()
    .metadata(metadata)
  );
```
...
val svix = Svix("AUTH_TOKEN")
val metadata = mapOf("svix.email" to "user-123@example.com")

svix.application.patch("user-123", ApplicationPatch(
    metadata = metadata
))
```
...
```ruby
svix = Svix::Client.new("AUTH_TOKEN")
svix.application.patch("user-123", Svix::ApplicationPatch.new({
    "metadata" => {
        "svix.email" => "user-123@example.com"
    }
}))
...
await svix.Application.PatchAsync("user-123", new ApplicationPatch(
    metadata: metadata
));
```
...
```shell
export SVIX_AUTH_TOKEN='AUTH_TOKEN'
svix application patch 'user-123' '{ "metadata": { "svix.email": "user-123@example.com" } }'
```
...
curl -X PATCH "https://api.us.svix.com/api/v1/app/user-123/" \
    -H  "Accept: application/json" \
    -H  "Content-Type: application/json" \
    -H  "Authorization: Bearer ${SVIX_AUTH_TOKEN}" \
    -d '{ "metadata": { "svix.email": "user-123@example.com" } }'
```
